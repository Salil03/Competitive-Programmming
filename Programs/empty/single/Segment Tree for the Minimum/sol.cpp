...
void update(vector<lll> &tree, lll value, lll idx, lll curr, lll l, lll r)
{
    if (l == r - 1)
    {
        tree[curr] = value;
...
lll query(vector<lll> &tree, lll idx, lll l, lll r, lll t_l, lll t_r)
{
    ...
    {
        lll m = (l + r) / 2;
        return min(query(tree, 2 * idx + 1, l, m, t_l, t_r), query(tree, 2 * idx + 2, m, r, t_l, t_r));
    }
}
