...
int main()
{
    ...
    lll n, q;
    cin >> n >> q;
    vector<vector<pair<lll, lll>>> queries(n);
    vector<lll> ans(n, -1);
    while (q--)
    {
        lll i, j, x;
        ...
        {
            queries[i - 1].push_back({j - 1, x});
            queries[j - 1].push_back({i - 1, x});
        }
    }
    for (lll i = 0; i < n; i++)
    {
    ...
    priority_queue<lll, vector<lll>, greater<lll>> unlock;
    for (lll i = 0; i < n; i++)
    {
        unlock.push(i);
    }
    while (!unlock.empty())
    {
        ...
        for (auto u : queries[curr])
        {
            if (ans[u.first] != -1)
            {
                ans[curr] = ans[curr] | (u.second ^ ans[u.first]);
            }
            else
            {
                unlock.push(u.first);
