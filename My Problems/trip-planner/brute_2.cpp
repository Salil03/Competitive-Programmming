...
int main()
{
	...
	lll start, end;
	cin >> start >> end;
	vector<vector<lll>> coprime(n + 1, vector<lll>());
	for (lll i = 1; i <= n; i++)
	{
		for (lll j = 1; j <= n; j++)
		{
	...
	for (lll k = 1; k <= m; k++)
	{
		for (lll idx = 1; idx <= n; idx++)
		{
			table[idx][1][0] = (table[idx][1][0] + sum_table[0][0]) % MOD;
			table[idx][1][1] = (table[idx][1][1] + sum_table[0][1]) % MOD;
			for (auto city : coprime[idx])
			{
				table[idx][1][0] = (table[idx][1][0] + table[city][0][1]) % MOD;
				table[idx][1][1] = (table[idx][1][1] + table[city][0][0]) % MOD;
			}
			sum_table[1][0] = (sum_table[1][0] + table[idx][1][0]) % MOD;
			sum_table[1][1] = (sum_table[1][1] + table[idx][1][1]) % MOD;
		}
		for (lll idx = 1; idx <= n; idx++)
		{
		...
		sum_table[0][0] = sum_table[1][0];
		sum_table[0][1] = sum_table[1][1];
		sum_table[1][0] = 0;
		sum_table[1][1] = 0;
	}
